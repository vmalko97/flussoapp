...
class Waybills
{
    ...
    {
        global $mysqli;
        $this->data = $mysqli->query("INSERT INTO wf_waybills (waybill_number, company_id, receiver_customer_id, 
        ...
        for ($i = 0; $i < count($dataTable); $i++) {
            $mysqli->query("INSERT INTO wf_waybill_records (product_id,product_name,units,quantity,
    ...
    public function getAsyncDataJsonById($id)
    {
        $json = "{\"async_data\": {";
        global $mysqli;
        $waybill = $mysqli->query("SELECT * FROM wf_waybills WHERE id=" . $id)->fetch_all(MYSQLI_ASSOC);
        $record = $mysqli->query("SELECT * FROM wf_waybill_records WHERE waybill_id=" . $id." ORDER BY position ASC")->fetch_all(MYSQLI_ASSOC);
    ...
    {
        global $mysqli;
        $this->data = $mysqli->query("SELECT wf_waybills.id, wf_waybills.waybill_number, wf_waybills.company_id, wf_customers.name AS receiver, wf_waybills.payer_customer_id, wf_waybills.same, wf_waybills.condition_of_sale, wf_waybills.date, wf_waybills.total_without_vat, wf_waybills.total, wf_waybills.vat, wf_waybills.finalize, wf_waybills.security_hash, wf_waybills.file_hash, wf_waybills.finalized_path FROM wf_waybills, wf_customers WHERE wf_waybills.receiver_customer_id = wf_customers.id ORDER BY waybill_number DESC")->fetch_all(MYSQLI_ASSOC);
        return $this->data;
    }
    public function getFinalizedComplexAll()
    {
        global $mysqli;
        $this->data = $mysqli->query("SELECT wf_waybills.id, wf_waybills.waybill_number, wf_waybills.company_id, wf_customers.name AS receiver, wf_waybills.payer_customer_id, wf_waybills.same, wf_waybills.condition_of_sale, wf_waybills.date, wf_waybills.total_without_vat, wf_waybills.total, wf_waybills.vat, wf_waybills.finalize, wf_waybills.security_hash, wf_waybills.file_hash, wf_waybills.finalized_path FROM wf_waybills, wf_customers WHERE wf_waybills.receiver_customer_id = wf_customers.id AND wf_waybills.finalize = '1' ORDER BY waybill_number DESC")->fetch_all(MYSQLI_ASSOC);
        return $this->data;
    }
    ...
    {
        global $mysqli;
        $this->data = $mysqli->query("DELETE FROM wf_waybills WHERE id=" . $id);
        $this->data = $mysqli->query("DELETE FROM wf_waybill_records WHERE waybill_id=" . $id);
        return $this->data;
    }
    ...
    {
        global $mysqli;
        $this->data = $mysqli->query("UPDATE wf_waybills SET finalize ='1', security_hash = '$security_hash', file_hash = '$file_hash', finalized_path = '$finalized_path' WHERE id=" . $id);
        return $this->data;
    }
    ...
    {
        global $mysqli;
        $this->data = $mysqli->query("SELECT * FROM wf_waybills WHERE file_hash = '$hash'")->fetch_all(MYSQLI_ASSOC);
        return $this->data;
    }
    public function getInfoBySecurityHash($hash)
    {
        global $mysqli;
        $this->data = $mysqli->query("SELECT * FROM wf_waybills WHERE security_hash = '$hash'")->fetch_all(MYSQLI_ASSOC);
        return $this->data;
    }
    public function getDataTableById($id){
        global $mysqli;
        $this->data = $mysqli->query("SELECT * FROM wf_waybill_records WHERE waybill_id =" . $id . " ORDER BY position ASC")->fetch_all(MYSQLI_ASSOC);
        return $this->data;
    }
    public function getAllByCustomerId($id)
    {
        global $mysqli;
        $this->data = $mysqli->query("SELECT * FROM wf_waybills WHERE receiver_customer_id = '$id'")->fetch_all(MYSQLI_ASSOC);
        return $this->data;
    }
    public function getAllFinalizedByCustomerId($id)
    {
        global $mysqli;
        $this->data = $mysqli->query("SELECT * FROM wf_waybills WHERE receiver_customer_id = '$id' AND finalize = 1 ")->fetch_all(MYSQLI_ASSOC);
        return $this->data;
    }
    public function getById($id)
    {
        global $mysqli;
        $this->data = $mysqli->query("SELECT * FROM wf_waybills WHERE id = '$id'")->fetch_all(MYSQLI_ASSOC);
        return $this->data;
    }
