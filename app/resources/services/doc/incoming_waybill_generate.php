...
$section->addTextRun($cellTextStyleCenter)->addText('Прибуткова накладна № ' . $async_data->{'waybill'}, ['size' => 12, 'bold' => true]);
...
if ($async_data->{'finalize'} == 0) {
    ...
    $objWriter->save($_SERVER['DOCUMENT_ROOT'] . '/temp/' . 'Прибуткова накладна ' . $async_data->{'waybill'} . ' TEMP.docx');
    ...
    $objWriter->save($_SERVER['DOCUMENT_ROOT'] . '/app/resources/finalized/incoming_waybills/' . 'Прибуткова накладна ' . $async_data->{'waybill'} . ' FINALIZED.docx');
    ...
    $finalize = [
        ...
        'file_hash' => hash_file('md5', $_SERVER['DOCUMENT_ROOT'] . '/app/resources/finalized/incoming_waybills/' . 'Прибуткова накладна ' . $async_data->{'waybill'} . ' FINALIZED.docx'),
        'finalized_path' => '/app/resources/finalized/incoming_waybills/' . 'Прибуткова накладна ' . $async_data->{'waybill'} . ' FINALIZED.docx'
    ];
}
$dataArr = array();
if ($async_function == "finalize_waybill") {
    ...
    $obj_secure->add($finalize['security_hash'], $finalize['file_hash'], "incoming_waybill", $async_data->{'id'});
    echo "success";
} else {
    ...
    if($async_function != "download_waybill") {
        if ($async_data->{'finalize'} == 0) {
            ...
            $obj_secure->add($finalize['security_hash'], $finalize['file_hash'], "incoming_waybill", $waybill);
        }
    }
    /** Download file link**/
    ...
    if ($async_data->{'finalize'} == 1) {
        ...
        $tempFile = 'Прибуткова накладна ' . $async_data->{'waybill'} . ' FINALIZED.docx';
        $response = [
            'response' => "success",
            'download' => '/app/resources/finalized/incoming_waybills/' . $tempFile,
            'tempFile' => $tempFile,
            'finalized' => true
        ];
        echo json_encode($response);
    } else {
        $tempFile = 'Прибуткова накладна ' . $async_data->{'waybill'} . ' TEMP.docx';
        $response = [
            'response' => "success",
