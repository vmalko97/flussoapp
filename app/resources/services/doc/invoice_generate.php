...
} elseif ($async_function == "finalize_invoice") {
    $async_request = $async->{'async_request'};
    $obj_invoice = new Invoices();
    $json = $obj_invoice->getFinalizeAsyncDataJsonById($async_request->{'id'});
    $async_dat = json_decode($json);
    $async_data = $async_dat->{'async_data'};
}else {
...
if ($async_data->{'finalize'} == 0) {
    ...
    $objWriter->save($_SERVER['DOCUMENT_ROOT'] . '/app/resources/finalized/invoices/' . 'Рахунок '.$async_data->{'invoice'}. ' FINALIZED.docx');
    ...
    $finalize = [
        ...
        'file_hash' => hash_file('md5', $_SERVER['DOCUMENT_ROOT'] . '/app/resources/finalized/invoices/' . 'Рахунок '.$async_data->{'invoice'}. ' FINALIZED.docx'),
        'finalized_path' => '/app/resources/finalized/invoices/' . 'Рахунок '.$async_data->{'invoice'}. ' FINALIZED.docx'
    ];
}
$dataArr = array();
if ($async_function == "finalize_invoice") {
    $obj_invoice->finalize($async_data->{'id'}, $finalize['security_hash'], $finalize['file_hash'], $finalize['finalized_path']);
    $obj_secure->add($finalize['security_hash'], $finalize['file_hash'], "invoice", $async_data->{'id'});
    echo "success";
} else {
    ...
    if ($async_function = "generate_invoice_and_waybill") {
        $obj_waybill->create($finalize['invoice_number'], $finalize['company_id'], $finalize['receiver_customer_id'],
            $finalize['payer_customer_id'], $finalize['same'], "Рахунок № " . $finalize['invoice_number'], $finalize['date'], $finalize['total_without_vat'],
    ...
    if($async_function != "download_invoice") {
        if ($async_data->{'finalize'} == 0) {
            ...
            $invoice = $obj_invoice->createAndFinalize($finalize['invoice_number'], $finalize['company_id'], $finalize['receiver_customer_id'],
                $finalize['payer_customer_id'], $finalize['same'], $finalize['date'], $finalize['total_without_vat'],
                $finalize['total'], $finalize['vat'], $finalize['written_by'], $finalize['finalize'], $finalize['security_hash'], $finalize['file_hash'], $finalize['finalized_path'], $dataArr);
            $obj_secure->add($finalize['security_hash'], $finalize['file_hash'], "invoice", $invoice);
        }
    }

    if ($async_data->{'finalize'} == 1) {

        $tempFile = 'Рахунок ' . $async_data->{'invoice'} . ' FINALIZED.docx';
        $response = [
            'response' => "success",
            'download' => '/app/resources/finalized/invoices/' . $tempFile,
            'tempFile' => $tempFile,
            'finalized' => true
        ];
