            ...
            <div class="row">
                <div class="col-lg-7 col-md-6 col-sm-12">
                    ...
                    <ul class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">Редагувати рахунок-фактуру</li>
                    </ul>
        ...
        <div class="container-fluid">
            <div class="row clearfix">
                <div class="col-lg-12">
                    <div class="card">
                        <div class="header">
                            <h2><strong>Редагувати рахунок-фактуру</strong></h2>
                        </div>
                        <?php
                        $id = filter_input(INPUT_GET, 'id', FILTER_SANITIZE_FULL_SPECIAL_CHARS);
                        if (empty($id) || !isset($id)) {
                            echo "<div class='body'>
                                <h2 class='card-inside-title'>Редагувати рахунок-фактуру</h2>
                                <div class='alert alert-danger'>Помилка! Не обрано рахунок для редагування.</div>
                            </div>
                            </div></div></div></div></div></section>";
                        }else{
                        $obj_invoice = new Invoices();
                        $invoice = $obj_invoice->getById($id);
                        $count_inv = count($invoice);
                        for ($inv = 0;
                        $inv < $count_inv;
                        $inv++) {
                        ?>
                        <div class="body">
                            <h5>Редагувати рахунок-фактуру</h5>
                            <div class="row clearfix">
                                <div class="col-md-12">
                                    ...
                                    <select class="form-control show-tick ms select2" name="provider"
                                        ...
                                        for ($i = 0; $i < $count; $i++) {
                                            ?>
                                            <option value="<?php echo $provider[$i]['id']; ?>" <?php if ($provider[$i]['id'] == $invoice[$inv]['company_id']) {
                                                echo "selected";
                                ...
                                <div class="col-md-12">
                                    ...
                                    <select class="form-control show-tick ms select2" name="customer"
                                        ...
                                        for ($j = 0; $j < $count_c; $j++) {
                                            ?>
                                            <option value="<?php echo $customer[$j]['id']; ?>" <?php if ($customer[$j]['id'] == $invoice[$inv]['receiver_customer_id']) {
                                                echo "selected";
                                ...
                                <div class="col-md-12">
                                    <p><b>Платник</b></p>
                                    <div id="payer_box" <?php if ($invoice[$inv]['same'] == 1) {
                                        ...
                                        <select class="form-control show-tick ms select2" name="payer"
                                            ...
                                            for ($k = 0; $k < $count_p; $k++) {
                                                ?>
                                                <option value="<?php echo $payer[$k]['id']; ?>" <?php if ($provider[$k]['id'] == $invoice[$inv]['payer_customer_id']) {
                                                    echo "selected";
                                    ...
                                    <div class="checkbox">
                                        <input type="checkbox" id="same" name="same"
                                               onchange="check_same()" <?php if ($invoice[$inv]['same'] == 1) {
                                            echo "checked";
                                        } ?>/>
                                ...
                                <div class="col-md-12">
                                    <p class="text-center"><b>Рахунок </b> <input type="text" class="form-control"
                                                                                  name="invoice"
                                                                                  value="<?= $invoice[$inv]['invoce_number'] ?>"/>
                                    </p>
                                </div>
                                <div class="col-md-12">
                                    <p class="text-center"><b>від </b> <input type="date" class="form-control"
                                                                              value="<?= $invoice[$inv]['date'] ?>"
                                                                              name="date"/></p>
                                </div>
                                <div class="col-md-12">
                                    <table class="table table-bordered text-center" id="product_list">
                                        ...
                                        <?php
                                        $dataTable = $obj_invoice->getDataTableById($id);
                                ...
                                <div class="col-md-12 text-right">
                                    <b>Разом без ПДВ:</b> <span
                                            id="sum_without_vat"><?= $invoice[$inv]['total_without_vat'] ?></span> <br/>
                                    <b>ПДВ:</b> <span id="sum_vat"><?= $invoice[$inv]['vat'] ?></span><br/>
                                    <b>Всього з ПДВ:</b> <span
                                            id="sum_with_vat"><?= $invoice[$inv]['total'] ?></span><br/>
                                    <br/>
                                </div>
                                <?php
                                ...
                                <div class="col-md-9">
                                    <input class="form-control" name="written_by"
                                           value="<?= $invoice[$inv]['written_by'] ?>""/>
                                </div>
                                <div class="col-md-12" hidden>
                                    <hr/>
                                    <div class="checkbox">
                                        <input type="checkbox" id="finalize" name="finalize" disabled/>
                                        <label for="finalize">Фіналізувати рахунок</label>
                                    </div>
                                    <hr/>
                                </div>
                                <div class="col-md-12">
                                    <button class="btn btn-block btn-success" onclick="save()">Зберегти
                                    </button>
                                </div>
                            </div>
                        </div>
...
<script>
    ...
    function save() {
        ...
        let written_by = $('input[name=written_by]').val();
        let finalize = null;
